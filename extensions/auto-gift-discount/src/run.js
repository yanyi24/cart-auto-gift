...
export function run(input) {
  ...
  if (buys.type === 'ALL_PRODUCTS') {
    ...
    if (rule === 'QUANTITY') {
      // 筛选出购物车中不包含礼品的商品
      const linesExceptedGifts = cartLines.filter(line => !allGifts.includes(removeGidStr(line.merchandise.id)));
      // 根据筛选结果找到对应的条件
      const condition = findObjectByValue(conditions, linesExceptedGifts.length);
    ...
    if (rule === 'AMOUNT') {
      // 计算购物车中不包含礼品商品的总金额
      const exceptedGiftsTotalAmount = cartLines
        .filter(line => !allGifts.includes(removeGidStr(line.merchandise.id)))
  ...
  if (buys.type === 'PRODUCTS') {
    ...
    if (rule === 'AMOUNT') {
      // 计算购物车中买入规则商品的总金额
      const buysTotalAmount = cartLines
        .filter(line => buysIds.includes(removeGidStr(line.merchandise.id)))
        .reduce((total, line) => total + Number(line.cost.totalAmount.amount), 0);
      // 根据总金额找到对应的条件
